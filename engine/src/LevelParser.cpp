	...
	for(TiXmlElement* e = pObjectElement->FirstChildElement(); e != NULL; e = e->NextSiblingElement()){
		if(e->Value() == std::string("object")){
			...
			if(strcmp(e->Attribute("type"), "XuxaBoss") == 0){
				...
				Game::Instance().addEnemy(dynamic_cast<XuxaBoss*>(pGameObject));
			}

			if(strcmp(e->Attribute("type"), "collider") == 0){
				Game::Instance().addCollider(dynamic_cast<SDLGameObject*>(pGameObject));
			}
